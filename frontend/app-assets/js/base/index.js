...
//             $(this).find("*[data-copy-click-id]").click(function(e) {
//                 e.stopPropagation();
//                 let copyText = itemId.text().trim();

//                 navigator.clipboard.writeText(copyText).then(() => {
//                     console.log('Đã sao chép: ' + copyText);
//                 }).catch(err => {
...
setTimeout(() => {
    $("*[data-copy-click-id]").click(function(e) {
        e.stopPropagation();
        let copyText = $(this).prev().text().trim(); // Lấy nội dung của phần tử trước nút copy và loại bỏ khoảng trắng không mong muốn

        if (copyText !== "") { // Chỉ sao chép nếu có nội dung
            navigator.clipboard.writeText(copyText).then(() => {
                console.log('Đã sao chép: ' + copyText);
            }).catch(err => {
                console.error('Lỗi khi sao chép: ', err);
