...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (checkToken("request_admin")) {
        ...
        // Tạo token mới
        $token = createToken(getEmailUser($user_id));

        // Cập nhật token mới
        pdo_execute("UPDATE `user` SET `user_token` = ? WHERE `user_id` = ?", [$token, $user_id]);

        response(true, 'Đã đăng xuất tất cả thiết bị thành công!');
    } else {
