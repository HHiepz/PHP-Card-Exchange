...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (checkToken("request_admin")) {
        ...
        pdo_execute("UPDATE `topup-order` SET `topup-order_status` = 'wait', `topup-order_cron` = 0 WHERE `topup-order_request_id` = ?", [$request_id]);

        response(true, "Reset đơn $request_id thành công");
    } else {
