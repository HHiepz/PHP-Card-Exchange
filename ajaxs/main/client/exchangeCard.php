    ...
    if (checkToken('request')) {
        ...
        foreach ($data as $row) {
            ...
            $sql = "INSERT INTO `card-data` SET 
                `card-data_telco`          = ?,    -- Nhà mạng
                `card-data_code`           = ?,    -- Mã thẻ
                `card-data_seri`           = ?,    -- Seri
                `card-data_amount`         = ?,    -- Mệnh giá
                `card-data_fee`            = ?,    -- Phí
                `card-data_amount_recieve` = ?,    -- Thực nhận
                `user_id`                  = ?,    -- ID người dùng
                `card-data_server`         = ?,    -- Server đối tác
                `card-data_request_id`     = ?,    -- Mã yêu cầu
